...
- **Badge poids estimé** : petit `Label` orange sous la courbe, tap → Réglages (`EstimatedWeightLabel`).
...
- **Concentration (abandonné le 2026-09-09, idée en backlog)** : l'affichage en mg/L (lignes sous Pic et Coucher dans la feuille de statut, note de toxicité) a été livré en M7.4 puis retiré : rien de dérivé du poids ne doit traverser l'App Group, et le héros comme la pastille gardent une seule info dominante (mg, statut). Si l'idée revient, elle vit dans la feuille de statut, jamais sous Cumul du jour (quantité ingérée), avec un nombre et son unité liés par une espace insécable.
...
- **Affichage (abandonné le 2026-09-09, idée en backlog)** : la section « Affichage » avec le choix de l'unité de la complication (mg / mg/L, liste à coche `ComplicationUnitPickerView`, haptique `.selection`) a été livrée en M7.4 puis retirée avec l'affichage en mg/L. Leçon de style à garder si un choix à coche revient : une ligne au style des autres réglages (valeur en accent à droite) qui pousse une liste, plutôt qu'un `Picker(.navigationLink)` dont le style watchOS met la valeur en sous-titre gris.
...
- **Unité** : les quatre familles affichent des mg entiers (« mg » dans l'ouverture de l'arc et dans la jauge du coin, « 142 mg · OK » en rectangulaire et inline, espace insécable entre nombre et unité). L'unité mg/L en complication est abandonnée le 2026-09-09 (idée en backlog). La galerie de debug garde un proxy du mode teinté (environnement `.accented` + désaturation — seul l'hôte teinte vraiment) et le grain seul.

...
- **Scrubber couronne** (le geste signature) : `.digitalCrownRotation` sur un offset temporel `−12 h … +6 h` (pas 15 min, `isHapticFeedbackEnabled`, `sensitivity: .low`). Pendant la rotation : la courbe passe à 96 pt, un curseur vertical suit, le nombre héros affiche la valeur à cet instant et l'heure s'affiche en surtitre (« à 23:00 » / « il y a 3 h »), la pastille reflète le statut projeté. Relâchement (1,2 s sans rotation) → retour spring à « maintenant ». Implémentation : `@State scrubOffsetMinutes: Double`, `TimelineView(.periodic(by: 60))` continue de faire vivre le niveau réel.
...
- Toutes les vues testées en taille de texte max et sur Ultra 3 (49 mm) + Series 11 42 mm (plus petit) : aucun texte tronqué, `minimumScaleFactor(0.8)` sur les nombres héros.
